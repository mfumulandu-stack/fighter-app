...
const trainersData = [
{id:1,name:“Freddie Roach”,country:“🇺🇸 USA”,style:“Boxing”,pupils:“Manny Pacquiao, Miguel Cotto”,gym:“Wild Card Boxing Club”,titles:28,rating:9.8,exp:35,emoji:“🥊”,accent:”#FFD700”,bio:“Einer der erfolgreichsten Boxing-Trainer aller Zeiten.”},
...
{id:3,name:“Rafael Cordeiro”,country:“🇧🇷 Brasilien”,style:“Muay Thai / MMA”,pupils:“Anderson Silva, Fabricio Werdum”,gym:“Kings MMA”,titles:15,rating:9.6,exp:28,emoji:“🔥”,accent:”#39FF14”,bio:“Weltklasse-Trainer mit über 30 Weltmeistern.”},
{id:4,name:“John Kavanagh”,country:“🇮🇪 Irland”,style:“MMA / BJJ”,pupils:“Conor McGregor, Gunnar Nelson”,gym:“SBG Ireland”,titles:10,rating:9.5,exp:20,emoji:“☘️”,accent:”#FF4500”,bio:“Brachte McGregor zur Weltelite.”},
];

const sportsData = {
“🏀 Basketball”:{color:”#FF6B35”,games:[{id:1,title:“Pickup Basketball”,location:“Tempelhof Courts, Berlin”,time:“Sa 15:00”,players:“4/10”,level:“Mittel”,host:“Kevin S.”,emoji:“🏀”},{id:2,title:“3on3 Tournament”,location:“Beach Courts München”,time:“So 12:00”,players:“8/12”,level:“Anfänger”,host:“Lena M.”,emoji:“🏆”}]},
“🎾 Tennis”:{color:”#39FF14”,games:[{id:1,title:“Casual Doubles”,location:“TC Rot-Weiß Berlin”,time:“So 10:00”,players:“2/4”,level:“Mittel”,host:“Anna K.”,emoji:“🎾”},{id:2,title:“Singles Sparring”,location:“Stadtpark Courts HH”,time:“Sa 14:00”,players:“1/2”,level:“Fortgeschritten”,host:“Felix R.”,emoji:“⚡”}]},
...
const allFighters=profile.name?[{id:0,name:profile.name,age:profile.age,city:profile.city,gym:profile.gym,weightClass:profile.weightClass,style:profile.style,wins:myStats.wins,losses:myStats.losses,draws:myStats.draws,ko:myStats.ko,emoji:“🥊”,accent:”#FF4500”,isMe:true,avatarUrl},…mockFighters]:mockFighters;
...
<div style={{minHeight:“100vh”,background:”#0a0a0a”,display:“flex”,flexDirection:“column”,alignItems:“center”,padding:“0 0 40px”}}>
<style>{css}</style>
<div style={{width:“100%”,maxWidth:420,padding:“32px 24px 0”,textAlign:“center”}}>
<div className=“fr fade-up” style={{fontSize:64,color:”#fff”,letterSpacing:6,lineHeight:1,textShadow:“0 0 40px #FF450088”}}>FIGHTER</div>
<div style={{color:”#FF4500”,fontSize:11,letterSpacing:7,marginTop:5,fontWeight:500}}>FINDE DEINEN GEGNER</div>
</div>
...
```
    ...
    {step===1&&<div style={{display:"flex",flexDirection:"column",gap:13}}>
      ...
      <div style={{display:"flex",justifyContent:"center",marginBottom:8}}>
        <label style={{cursor:"pointer",textAlign:"center"}}>
          ...
          <div style={{width:90,height:90,borderRadius:"50%",background:"#1a1a1a",border:`2px dashed ${avatarPreview?"#FF4500":"#333"}`,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",margin:"0 auto"}}>
            ...
            : <div style={{textAlign:"center"}}><div style={{fontSize:28}}>📸</div><div style={{color:"#444",fontSize:10,marginTop:4}}>Foto hinzufügen</div></div>}
          </div>
          {!uploadingPhoto && <div style={{color:"#FF4500",fontSize:11,marginTop:6,fontWeight:600}}>Profilbild hochladen</div>}
        </label>
      </div>
    ...
    {step===2&&<div style={{display:"flex",flexDirection:"column",gap:13}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:7,marginTop:4}}>
        {FIGHT_STYLES.map(s=><button key={s} onClick={()=>setProfile(p=>({...p,style:s}))} style={{padding:"7px 13px",borderRadius:4,border:`1px solid ${profile.style===s?"#FF4500":"#2a2a2a"}`,background:profile.style===s?"#FF450022":"#111",color:profile.style===s?"#FF4500":"#666",fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:700,cursor:"pointer",transition:"all 0.2s"}}>{s}</button>)}
      </div>
    </div>}
      ...
      <div style={{display:"flex",gap:7}}>
        {[["wins","SIEGE","#39FF14"],["losses","NIEDER","#FF4500"],["draws","UNENTSCH","#FFD700"],["ko","KOs","#FF6B35"]].map(([key,label,color])=>(
          <div key={key} style={{flex:1,textAlign:"center"}}>
            ...
            <input type="number" min="0" value={myStats[key]} onChange={e=>setMyStats(s=>({...s,[key]:parseInt(e.target.value)||0}))} style={{width:"100%",background:"#111",border:"1px solid #2a2a2a",borderRadius:6,padding:"9px 3px",color:"#fff",fontSize:20,textAlign:"center",fontFamily:"'Rajdhani',sans-serif"}}/>
          </div>
        ))}
      </div>
    </div>}
...
```
  {saveMsg&&<div style={{position:"fixed",top:60,left:"50%",transform:"translateX(-50%)",background:"#1a1a1a",border:"1px solid #FF4500",borderRadius:20,padding:"8px 20px",color:"#fff",fontSize:13,zIndex:200,fontWeight:600}}>{saveMsg}</div>}
  ...
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 18px 5px",flexShrink:0,borderBottom:"1px solid #111"}}>
    <div style={{color:"#333",fontSize:11}}>❌ {swipeStats.declines}</div>
    <div className="fr" style={{fontSize:28,color:"#fff",letterSpacing:5,textShadow:"0 0 14px #FF450055"}}>FIGHTER</div>
    <div style={{color:"#FF4500",fontSize:11}}>⚔️ {swipeStats.challenges}</div>
  </div>
  ...
  <div style={{flex:1,overflowY:"auto",paddingBottom:65}}>
    ...
    {tab==="swipe"&&(
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",paddingTop:8}}>
        <div style={{width:"calc(100% - 24px)",maxWidth:380,margin:"0 0 8px",background:"#111",borderRadius:10,padding:"9px 12px",border:"1px solid #1a1a1a",display:"flex",alignItems:"center",gap:9}}>
          {avatarPreview
            ? <img src={avatarPreview} style={{width:36,height:36,borderRadius:"50%",objectFit:"cover",border:"2px solid #FF4500"}} alt="me"/>
            : <div style={{fontSize:20,width:36,height:36,borderRadius:"50%",background:"#222",display:"flex",alignItems:"center",justifyContent:"center"}}>🥊</div>}
          <div style={{flex:1}}>
            <div style={{color:"#fff",fontWeight:700,fontSize:13}}>{profile.name}, {profile.age} · {profile.city}</div>
            <div style={{color:"#FF4500",fontSize:11,marginTop:1}}>{profile.style} · {profile.weightClass?.split(" (")[0]}</div>
          </div>
        ...
        <div style={{position:"relative",width:330,height:430,flexShrink:0}}>
          {cards.length===0?(
            <div style={{width:"100%",height:"100%",borderRadius:16,background:"#111",border:"2px dashed #1a1a1a",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:11}}>
              ...
              <button onClick={()=>{setCards([...mockFighters]);setSwipeStats({challenges:0,declines:0});setMatches([]);}} style={{marginTop:6,padding:"9px 22px",borderRadius:6,background:"linear-gradient(135deg,#FF4500,#FF6B35)",color:"#fff",border:"none",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:15,cursor:"pointer"}}>Nochmal</button>
            </div>
          ):cards.map((f,idx)=>{
            ...
            return(
              <div key={f.id} onMouseDown={isTop?handleDragStart:undefined} onTouchStart={isTop?handleDragStart:undefined}
                ...
                {isTop&&<>
                  <div style={{position:"absolute",top:18,left:16,border:"3px solid #39FF14",borderRadius:5,padding:"2px 8px",color:"#39FF14",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:22,letterSpacing:3,transform:"rotate(-18deg)",opacity:fightOp,transition:dragging?"none":"opacity 0.12s"}}>FIGHT</div>
                  <div style={{position:"absolute",top:18,right:16,border:"3px solid #FF4500",borderRadius:5,padding:"2px 8px",color:"#FF4500",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:22,letterSpacing:3,transform:"rotate(18deg)",opacity:passOp,transition:dragging?"none":"opacity 0.12s"}}>PASS</div>
                </>}
                ...
                <div style={{padding:"10px 14px 14px",background:"rgba(0,0,0,0.55)",borderTop:`1px solid ${f.accent}18`}}>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <div><div className="fr" style={{color:"#fff",fontSize:24,letterSpacing:1.5,lineHeight:1}}>{f.name}</div><div style={{color:f.accent,fontSize:11,fontWeight:700,marginTop:1}}>{f.style?.toUpperCase()}</div></div>
            ...
            <div style={{display:"flex",gap:7,overflowX:"auto"}}>
              {matches.map(f=>(
                <div key={f.id} style={{textAlign:"center",flexShrink:0}}>
                  <div style={{width:42,height:42,borderRadius:6,background:"#111",border:`2px solid ${f.accent}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{f.emoji}</div>
        ...
        <div style={{background:"#111",borderRadius:14,padding:"16px",border:"1px solid #1a1a1a",marginBottom:11,textAlign:"center"}}>
          <div style={{position:"relative",display:"inline-block",marginBottom:10}}>
            <label style={{cursor:"pointer"}}>
              <input type="file" accept="image/*" onChange={handlePhotoUpload} style={{display:"none"}}/>
              <div style={{width:80,height:80,borderRadius:"50%",background:"#1a1a1a",border:`3px solid ${avatarPreview?"#FF4500":"#333"}`,overflow:"hidden",margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"center"}}>
                {uploadingPhoto?<div style={{fontSize:24}} className="spin">⏳</div>
                :avatarPreview?<img src={avatarPreview} style={{width:"100%",height:"100%",objectFit:"cover"}} alt="avatar"/>
                :<div style={{fontSize:32}}>👤</div>}
              </div>
              <div style={{position:"absolute",bottom:0,right:0,background:"#FF4500",borderRadius:"50%",width:24,height:24,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12}}>📷</div>
            </label>
          </div>
          <div className="fr" style={{color:"#fff",fontSize:24,letterSpacing:2}}>{profile.name}</div>
          <div style={{color:"#FF4500",fontSize:13,fontWeight:600,marginTop:2}}>{profile.style} · {profile.weightClass?.split(" (")[0]}</div>
          <div style={{color:"#444",fontSize:11,marginTop:3}}>📍 {profile.city} · 🏋️ {profile.gym}</div>
          {profile.bio&&<div style={{color:"#555",fontSize:12,marginTop:6,fontStyle:"italic"}}>"{profile.bio}"</div>}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:9}}>
          {[["KO / TKO",myStats.ko,"#FF4500",`KO-Rate: ${koRate}%`,koRate],["SIEGRATE",`${winRate}%`,"#39FF14",`${totalFights} Kämpfe`,winRate]].map(([label,val,color,sub,pct])=>(
            <div key={label} style={{background:"#111",borderRadius:11,padding:"13px",border:`1px solid ${color}15`}}>
              ...
              <div style={{marginTop:6,height:3,background:"#1a1a1a",borderRadius:2}}><div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${color},${color}77)`,borderRadius:2}}/></div>
            </div>
          ))}
        </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:5}}>
            {[["wins","SIEGE","#39FF14"],["losses","NIEDER","#FF4500"],["draws","UNENTSCH","#FFD700"],["ko","KOs","#FF6B35"]].map(([key,label,color])=>(
              <div key={key} style={{textAlign:"center"}}>
                <div style={{color:"#2a2a2a",fontSize:8,marginBottom:3}}>{label}</div>
            ...
            <div key={i} style={{background:"#111",borderRadius:12,padding:"13px",border:"1px solid #1a1a1a"}}>
              <div style={{display:"flex",gap:11,alignItems:"flex-start"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{display:"flex",gap:4,marginTop:6,flexWrap:"wrap"}}>{gym.styles.map(s=><FTag key={s} color="#FF450012" text={s} accent="#FF4500"/>)}</div>
                </div>
              </div>
          ...
          <div style={{display:"flex",alignItems:"flex-end",gap:5,marginBottom:13,justifyContent:"center"}}>
            {[rankedFighters[1],rankedFighters[0],rankedFighters[2]].map((f,i)=>{
              ...
              return(<div key={f.id} style={{flex:1,maxWidth:105,display:"flex",flexDirection:"column",alignItems:"center"}}>
                {f.avatarUrl?<img src={f.avatarUrl} style={{width:40,height:40,borderRadius:"50%",objectFit:"cover",border:`2px solid ${colors[i]}`,marginBottom:3}} alt={f.name}/>:<div style={{fontSize:24,marginBottom:2}}>{f.emoji}</div>}
                <div style={{color:f.isMe?"#FF4500":"#ccc",fontSize:11,fontWeight:700,textAlign:"center"}}>{f.name.split(" ")[0]}</div>
            ...
            return(<div key={f.id} style={{background:f.isMe?"#FF450010":"#111",borderRadius:9,padding:"10px 12px",border:`1px solid ${f.isMe?"#FF450033":i<3?`${rankColors[i]}1a`:"#1a1a1a"}`,display:"flex",alignItems:"center",gap:9}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  ...
                  {f.isMe&&<div style={{background:"#FF450018",border:"1px solid #FF450033",borderRadius:3,padding:"1px 4px",color:"#FF4500",fontSize:8,fontWeight:700}}>ICH</div>}
                </div>
              ...
              <div style={{padding:"14px"}}>
                <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                  <div style={{position:"relative",flexShrink:0}}>
                    ...
                    <div style={{position:"absolute",bottom:-5,right:-5,background:i===0?"#FFD700":i===1?"#C0C0C0":i===2?"#CD7F32":"#1a1a1a",borderRadius:10,padding:"1px 5px"}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:"flex",justifyContent:"space-between"}}>
                      ...
                      <div style={{textAlign:"right"}}>
                        <div style={{display:"flex",alignItems:"center",gap:2}}><span style={{color:"#FFD700"}}>★</span><span style={{color:"#fff",fontWeight:700,fontSize:14}}>{t.rating}</span></div>
                        <div style={{color:"#333",fontSize:10}}>{t.exp} Jahre</div>
                      </div>
                    </div>
                    <div style={{color:"#444",fontSize:11,marginTop:2}}>{t.country} · {t.gym}</div>
                  </div>
                </div>
                <div style={{marginTop:9,color:"#555",fontSize:12,borderTop:"1px solid #1a1a1a",paddingTop:8}}>{t.bio}</div>
                <div style={{marginTop:8,background:"#0e0e0e",borderRadius:7,padding:"7px 10px"}}>
                  <div style={{color:"#333",fontSize:9,letterSpacing:1,marginBottom:3}}>BEKANNTE SCHÜLER</div>
                  <div style={{color:"#666",fontSize:12,fontWeight:600}}>{t.pupils}</div>
                </div>
                <div style={{marginTop:8,height:3,background:"#1a1a1a",borderRadius:2}}><div style={{height:"100%",width:`${(t.rating/10)*100}%`,background:`linear-gradient(90deg,${t.accent},${t.accent}66)`,borderRadius:2}}/></div>
              </div>
            </div>
          ))}
        </div>
    ...
    {tab==="sports"&&(
      <div style={{padding:"10px 13px 16px",maxWidth:420,margin:"0 auto"}}>
        ...
        <div style={{color:"#444",fontSize:12,marginBottom:11}}>Finde Events & Pickup-Games</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:14}}>
          {Object.keys(sportsData).map(sport=>{
            const {color}=sportsData[sport],isSelected=selectedSport===sport;
            return(<button key={sport} onClick={()=>setSelectedSport(sport)} style={{padding:"12px 10px",borderRadius:11,background:isSelected?`${color}22`:"#111",border:`1px solid ${isSelected?color:"#1e1e1e"}`,cursor:"pointer",transition:"all 0.2s",textAlign:"left"}}>
                ...
                <div style={{padding:"13px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <div style={{display:"flex",gap:10,alignItems:"center"}}>
                      ...
                      <div>
                        <div style={{color:"#fff",fontWeight:700,fontSize:14}}>{game.title}</div>
                        <div style={{color:"#444",fontSize:11}}>📍 {game.location}</div>
                      </div>
                    </div>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <div style={{display:"flex",gap:10}}>
                      <div style={{color:"#555",fontSize:11}}>🕐 {game.time}</div>
                      <div style={{color:"#555",fontSize:11}}>👤 {game.host}</div>
                    </div>
                    <div style={{color:full?"#FF4500":color,fontSize:11,fontWeight:700}}>{game.players}</div>
                  </div>
                  <div style={{height:4,background:"#1a1a1a",borderRadius:3,marginBottom:9}}><div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${color},${color}77)`,borderRadius:3}}/></div>
  ...
  <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#0d0d0d",borderTop:"1px solid #161616",display:"flex",height:58,zIndex:50}}>
    {tabs.map(([id,icon,label])=>(
      <button key={id} onClick={()=>setTab(id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"transparent",border:"none",cursor:"pointer",gap:1,borderTop:tab===id?"2px solid #FF4500":"2px solid transparent",transition:"all 0.2s"}}>
        ...
        <div style={{color:tab===id?"#FF4500":"#222",fontSize:9,fontFamily:"'DM Sans',sans-serif",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5}}>{label}</div>
      </button>
    ))}
  </div>
  ...
  {matchedFighter&&(
    <div onClick={()=>setMatchedFighter(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.93)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:100,gap:12,animation:"slideIn 0.3s ease"}}>
      ...
      <div className="fr" style={{color:"#fff",fontSize:24,letterSpacing:2}}>{matchedFighter.name}</div>
      <div style={{color:matchedFighter.accent,fontSize:12,fontWeight:700}}>{matchedFighter.style} · {matchedFighter.weightClass}</div>
      <button style={{marginTop:5,padding:"11px 26px",borderRadius:6,background:"linear-gradient(135deg,#FF4500,#FF6B35)",color:"#fff",border:"none",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:16,letterSpacing:2,cursor:"pointer"}}>Fight anfordern 🥊</button>
...
function FLabel({children}){return <div style={{color:”#555”,fontSize:11,fontWeight:600,letterSpacing:1.5,textTransform:“uppercase”}}>{children}</div>;}
...
style={{width:“100%”,background:”#111”,border:“1px solid #1e1e1e”,borderRadius:8,padding:`12px ${icon?"12px 12px 34px":"13px"}`,color:”#fff”,fontSize:15,fontFamily:”‘DM Sans’,sans-serif”,transition:“border-color 0.2s”}}
