  ...
  <div style={{flex:1,overflowY:"auto",paddingBottom:65}}>
          ...
          <div style={{display:"flex",alignItems:"flex-end",gap:5,marginBottom:13,justifyContent:"center"}}>
            {[rankedFighters[1],rankedFighters[0],rankedFighters[2]].map((f,i)=>{
              ...
              return(<div key={f.id} style={{flex:1,maxWidth:105,display:"flex",flexDirection:"column",alignItems:"center"}}>
                ...
                <div style={{color:f.isMe?"#E03000":"#ccc",fontSize:11,fontWeight:700,textAlign:"center"}}>{f.name.split(" ")[0]}</div>
            ...
            return(<div key={f.id} style={{background:f.isMe?"#FF450010":"#111",borderRadius:9,padding:"10px 12px",border:`1px solid ${f.isMe?"#FF450033":i<3?`${rankColors[i]}1a`:"#1a1a1a"}`,display:"flex",alignItems:"center",gap:9}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  <div style={{color:f.isMe?"#E03000":"#fff",fontWeight:700,fontSize:13}}>{f.name}</div>
    ...
    {tab==="trainer"&&(
      <div style={{padding:"10px 13px 16px",maxWidth:420,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:7,marginBottom:11}}>
          {trainerStyles.map(s=><button key={s} onClick={()=>setTrainerFilter(s)} style={{flexShrink:0,padding:"5px 11px",borderRadius:16,background:trainerFilter===s?"#d97706":"#111",border:`1px solid ${trainerFilter===s?"#d97706":"#222"}`,color:trainerFilter===s?"#000":"#555",fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all 0.2s"}}>{s==="All"?"Alle":s}</button>)}
        </div>
          ...
          {filteredTrainers.map((t,i)=>(
            <div key={t.id} style={{background:"#fff",borderRadius:13,border:`1px solid ${t.accent}22`,overflow:"hidden"}}>
              ...
              <div style={{padding:"14px"}}>
                ...
                <div style={{marginTop:9,color:"#666",fontSize:12,borderTop:"1px solid #1a1a1a",paddingTop:8}}>{t.bio}</div>
                <div style={{marginTop:8,background:"#fff",borderRadius:7,padding:"7px 10px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:14}}>
          {Object.keys(sportsData).map(sport=>{
            ...
            return(<button key={sport} onClick={()=>setSelectedSport(sport)} style={{padding:"12px 10px",borderRadius:11,background:isSelected?`${color}22`:"#111",border:`1px solid ${isSelected?color:"#1e1e1e"}`,cursor:"pointer",transition:"all 0.2s",textAlign:"left"}}>
              ...
              <div style={{color:isSelected?color:"#666",fontWeight:700,fontSize:13}}>{sport.split(" ").slice(1).join(" ")}</div>
              <div style={{color:"#aaa",fontSize:10,marginTop:2}}>{sportsData[sport].games.length} Events</div>
            </button>);
          })}
        </div>
